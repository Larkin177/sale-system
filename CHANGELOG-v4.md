...
### 11.14 推广二维码美化 `Link.vue`
  
| 改动 | 说明 |
|---|---|
| 卡片重设计 | 纯色扁平风格，移除渐变，圆角方框徽章居中放 logo |
| 品牌名 | "CC-INSTALLER" 全大写置于二维码下方，装饰线分隔 |
| 保存二维码 | 使用 `html2canvas` 截图整个二维码区域（含 logo + 品牌名），下载与显示一致 |
| Logo 渲染 | CSS 白底圆角方框叠加，替代 `qr-code-styling` 内嵌（解决透明logo看不清的问题） |
| 图标修复 | `Smartphone` → `Iphone`（Element Plus 无 Smartphone 图标） |
| 模板结构 | 修复标签闭合顺序错误 |

### 11.15 管理端二维码预览 `Config.vue`

| 改动 | 说明 |
|---|---|
| 真实链接 | 预览二维码改用 `site_url`（不再使用 `example.com` 演示地址） |
| 尺寸统一 | 预览改用 260×260px，与销售端完全一致 |
| 保存按钮 | 新增「保存预览二维码」按钮，`html2canvas` 截图保存 |
| 样式统一 | Logo 方框大小、品牌名间距、字号与销售端对齐 |
