...
#### 产品套餐管理系统
- 新增 `product_packages` 表，支持同一产品多平台多套餐管理
- 管理端新增**套餐管理**页面：配置套餐名称、平台、版本、价格范围、下载链接、授权码
- 客户端购买流程：手机验证 → 选择平台 → 选择套餐 → 确认支付
- 销售推广链接按套餐生成，自定义价格受套餐价格范围约束
- 支持销售链接直达（跳过套餐选择，格式：`/pay?s=推广码&pkg=套餐ID&p=价格`）

#### 文件上传功能
- 管理端所有图片/视频/文件输入框支持直接上传
- 支持格式：jpg/png/gif/webp/mp4/avi/mov/zip/pdf/doc/xls 等
- 最大文件大小：100MB，上传后自动保存并实时预览

#### 站点设置增强
- 产品页标题、副标题、提示语可在管理端配置
- 发货消息模板新增占位符：`{product_name}`、`{package_name}`、`{platform}`、`{version}`

### 修复
- **上传缩略图不显示**：Vite 代理添加 `/uploads` 路径转发
- **上传后设置未保存**：上传成功后自动调用保存接口
- **价格验证报错**：删除 PayController 中多余的价格范围校验
- **Order 实体缺少字段**：补充 `packageId`、`packageName`、`platform`
- **发货消息硬编码**：从订单关联的套餐获取下载链接和授权配置

### 数据库变更
- 新增 `product_packages` 表
- `orders` 表新增 `package_id`、`package_name`、`platform` 字段
- `system_config` 新增产品页配置项
- 详见 `sql/v4-product-packages.sql`
...
#### 支付回调实现
- **修复**: 实现微信和支付宝支付回调的实际处理逻辑
- **文件**: `backend/src/main/java/com/sales/controller/WebhookController.java`
- **功能**:
  - 添加签名验证
  - 更新订单状态
  - 计算分润
...
#### 分润计算准确性
- **修复**: 修复AdminStatsController中的分润计算逻辑
- **文件**: `backend/src/main/java/com/sales/controller/AdminStatsController.java`
- **功能**:
  - 从commissions表查询实际分润金额
  - 添加本月分润统计

#### 下载链接安全
- **修复**: 加固下载链接安全性
- **文件**: `backend/src/main/java/com/sales/controller/DownloadController.java`
- **功能**:
  - 添加下载次数限制（最多10次）
  - 添加token格式验证
  - 添加下载IP记录
...
- **修复**: 优化N+1查询问题
- **文件**:
  - `backend/src/main/java/com/sales/service/OrderService.java`
  - `backend/src/main/java/com/sales/mapper/OrderMapper.java`
- **功能**:
  - 使用JOIN查询替代N+1查询
  - 使用SQL聚合查询替代Java层聚合
  - 添加数据库索引优化
...
- **修复**: 添加销售详情页面
- **文件**:
  - `frontend/src/views/admin/SalesDetail.vue` (新增)
  - `frontend/src/views/admin/Sales.vue`
...
- **修复**: 优化前端交互体验
- **文件**:
  - `frontend/src/views/sales/Dashboard.vue`
- **功能**:
  - 添加订单状态显示优化
  - 添加更详细的状态文本
...
本次更新主要修复了系统中的关键问题，包括支付回调、分润计算、下载链接安全等，并完善了销售管理功能、用户体验、批量操作和日志监控。系统现在更加稳定、安全和易用。

建议在生产环境部署前，先在测试环境进行充分测试，并根据实际需求进行相应的配置调整。
